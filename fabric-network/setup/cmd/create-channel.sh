...
createChannel(){
    rm -rf ./channel-blocks/*
    setGlobalsForPeer0Org1

    peer channel create -o localhost:7050 -c $CHANNEL_NAME \
    --ordererTLSHostnameOverride orderer.alanhou.org \
    -f ./channel-artifacts/${CHANNEL_NAME}.tx --outputBlock ./channel-blocks/${CHANNEL_NAME}.block \
...
joinChannel(){
    setGlobalsForPeer0Org1
    peer channel join -b ./channel-blocks/${CHANNEL_NAME}.block
...
updateAnchorPeers(){
    setGlobalsForPeer0Org1
    peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer.alanhou.org -c $CHANNEL_NAME -f ./channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA
